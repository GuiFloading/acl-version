...
%%
%%
\documentclass[manuscript, anonymous=true, review=true]{acmart}
\usepackage{graphicx}
\usepackage{makecell}
\usepackage{pgf}
\usepackage{amsmath}
...
\email{gui.p.oliveira@proton.me}
\email{g.pintodeoliveira@student.fontys.nl}
\orcid{0009-0002-7830-2845}
\affiliation{%
...
\keywords{AI, Neural Networks, Few-shot Classification, Ensemble Models, Prototypical Networks}
...
\begin{figure}[h]
  \centering
  \resizebox{.5\linewidth}{!}{\input{images/boxplots_unclean_vs_cleaned.pgf}}
  \caption{Aggregate results per k-shot between architectures and preprocessing methods}
  \Description{
    Five box plots showing the accuracy per k-shot, for all five architecture variations. The image also
    indicates that the blue boxes represent the unclean data, and the orange boxes represent the cleaned data.
  }
...
\begin{figure}[h]
  \centering
  \resizebox{.6\linewidth}{!}{\input{images/boxplots_unclean_vs_cleaned_by_dataset.pgf}}
  \caption{Aggregate results per class between architectures and preprocessing methods}
  \Description{
    Five box plots showing the accuracy per class, for all five architecture variations. The image also
    indicates that the blue boxes represent the unclean data, and the orange boxes represent the cleaned data.
  }
...
\begin{figure}[h]
  \centering
  \resizebox{.6\linewidth}{!}{\input{images/accuracy_vs_k_shot_trend.pgf}}
  \caption{Correlation between accuracy per model and K-shot}
  \Description{
    A line graph showcasing the relation between Mean Accuracy and k-shot. There are 5 lines in total, each one representing a different model.
  }
...
The data provided by Solid Sustainability Research was labelled manually, with the help of ChatGPT's
API, with clear guidelines on how they were labelled. Therefore, there might be some mislabelled
projects. Furthermore, the results observed on the dataset used may not reflect on other datasets of
similar complexity.
...
\begin{acks}
 thank the researchers from Solid Sustainability Research for providing the busi-
ness context and data, as well their incredible, thoughful and patient guidence.
It has been a pleasure and an honour to contribute to a project for which its
mission means so much. My appreciations also extend to my colleagues at the
Master of Applied Sciences at Fontys ICT for providing me with good feedback
on how to improve this paper, including general encouragment and interest.
\end{acks}
